...
export default class CarouselWebPart extends BaseClientSideWebPart<
  ICarouselWebPartProps
> {
  ...
  private async loadLists(): Promise<IPropertyPaneDropdownOption[]> {
    ...
    try {
      ...
      )} -  please check if site url if valid.`;
      this.context.propertyPane.refresh();
    }
    return _lists;
  }
  ...
  protected async onPropertyPaneFieldChanged(
    ...
    try {
      ...
      if (propertyPath === "siteUrl" && newValue) {
        ...
        this.listsDropdownDisabled = false;
        this.properties.list =
          this.lists.length > 0 ? this.lists[0].key.toString() : undefined;
        this.context.propertyPane.refresh();
        this.render();
      } else {
        super.onPropertyPaneFieldChanged(propertyPath, oldValue, newValue);
